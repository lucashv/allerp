...
.cssDIV_VALORES {
position: absolute; top: 200px;  width: 900px; height: 150px;	
margin-top: -270px; margin-left: -445px; display:block; z-index:3;}
  ...
  <td width="<?php echo $_SESSION['largIFRAME']; ?>" height="<?php echo $_SESSION['altIFRAME']; ?>">
    ...
    <table cellspacing="0" cellpadding="0" border="1" width="65%" bgcolor="white" style="text-align:left;">
      ...
      <tr><td><table width="100%" cellpadding="0" cellspacing="2"><tr>
        ...
        <td title="Duplicar comissão" align="center" onmouseout="this.style.backgroundColor='white'" 
        onmouseover="this.style.backgroundColor='#A9B2CA'" onclick="duplicar();" >
          <img src="images/duplicar.png" />
        </td>

        <td title="Valores para corretor interno" align="center" onmouseout="this.style.backgroundColor='white'" 
        onmouseover="this.style.backgroundColor='#A9B2CA'" onclick="valores(1);" >
          <img src="images/percentual1.png" />
        </td>
        
        <td title="Valores para corretor externo" align="center" onmouseout="this.style.backgroundColor='white'" 
        onmouseover="this.style.backgroundColor='#A9B2CA'" onclick="valores(2);" >
          <img src="images/percentual2.png" />
        </td>    
...
aCMPS[0]='txtNOME;Digite a descrição do tipo';
...
ajax.criar('ajax/ajaxTIPOS_COMISSAO_REPRESENTANTE.php?acao=lerREGS&ativos='+
...
if ( ajax.terminouLER() ) {
  ...
  titulo.innerHTML = '&nbsp;&nbsp;<font size="+1">Tipos de comissão CORRETOR</font>&nbsp;&nbsp;&nbsp;&nbsp;';
...
function incluirREG() {
showAJAX(1);
ajax.criar('ajax/ajaxTIPOS_COMISSAO_REPRESENTANTE.php?acao=incluirREG', desenhaJanelaREG);
}
...
showAJAX(1);
ajax.criar('ajax/ajaxTIPOS_COMISSAO_REPRESENTANTE.php?acao=editarREG&vlr=' + id, desenhaJanelaREG);
}
...
if (nomeCMP!='todos')         {
  ...
  /* tudo ok, aciona gravacao do registro, concatena campos em uma variavel */
	showAJAX(1);
	ajax.criar('ajax/ajaxTIPOS_COMISSAO_REPRESENTANTE.php?acao=gravar&vlr=' + cmps, gravou);
}
...
ajax.criar('ajax/ajaxTIPOS_COMISSAO_REPRESENTANTE.php?acao=mudarSITUACAO&vlr=' + id, '', 0);
...
ajax.criar('ajax/ajaxTIPOS_COMISSAO_REPRESENTANTE.php?acao=valores&vlr=' + id+'&qual='+qual+'&desc='+desc, '', 0);
...
/*******************************************************************************/
function editaCOMISSAO(idPRODUTO, interno_externo, comissaoPorQtdeVidas, parcela)     {
...
showAJAX(1);
ajax.criar('ajax/ajaxTIPOS_COMISSAO_REPRESENTANTE.php?acao=editarCOMISSAO&prod=' + idPRODUTO+'&qual='+interno_externo+'&parcela='+parcela+
    '&vlr='+vlr+'&idCOMISSAO='+idCOMISSAO+'&comiQtdeVidas='+comissaoPorQtdeVidas, '', 0);
showAJAX(0);

valores(interno_externo);
}
...
ajax.criar('ajax/ajaxTIPOS_COMISSAO_REPRESENTANTE.php?acao=padrao&vlr=' + id, '', 0);
if (ajax.ler().indexOf('ok')==-1) 
  alert('Erro ao gravar!!! \n\n' + ajax.ler());
...
/*******************************************************************************/
function duplicar() {
idREG = document.getElementById('SELECAO').value; 
if (idREG=='') { alert('Selecione um registro');return;}

var tab30 = document.getElementById("tabREGs");
var nomeCOMI='* ERRO *';
idESQUEMA=-1;
for(var lima=0;lima < tab30.rows.length; lima++) {
	if ( tab30.rows[lima].id == idREG ) { 
    nomeCOMI = tab30.rows[lima].cells[2].innerHTML+' ('+tab30.rows[lima].cells[0].innerHTML+')';
    idESQUEMA = tab30.rows[lima].cells[0].innerHTML; 
    break;
  }
}

if (! confirm('Os percentuais da comissão '+nomeCOMI+' serão repetidos em um novo esquema, cujo nome você escolherá a seguir \n\n'+
              'Use esta opção caso precise criar uma nova comissão com poucas modificações em relação à comissão acima.\n\n'+
               'Não crie comissões desnecessárias.\n\nContinua?\n\n') )
  return;

resp = prompt('Qual o nome do novo esquema criar usando como base a comissão '+nomeCOMI+'\n\n'+
              'NAO USE PONTO, HÍFEN, NENHUM CARACTER ESPECIAL - USE SOMENTE LETRAS NÚMEROS E DATAS\n\n', '');

if (resp==null) return;
if (resp.rtrim()=='') return;

if (! confirm('Confirma duplicar '+nomeCOMI+' para \n\n'+resp+'\n\n'))
  return;

if (idESQUEMA==-1) return;
showAJAX(1)
ajax.criar('ajax/ajaxTIPOS_COMISSAO_REPRESENTANTE.php?acao=duplicar&vlr=' + idESQUEMA+'&novo='+resp, '', 0);
showAJAX(0)
resp=ajax.ler();
