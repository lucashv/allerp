...
<form id="frmGRUPOS_VENDA" name="frmGRUPOS_VENDA" autocomplete="off" action="" >
...
var aCMPS=new Array();
aCMPS[0]='txtNOME;Digite o nome do grupo';
aCMPS[1]='txtCOMISSAO_REPRESENTANTE;Identifique o tipo de comissão';
...
if (cmp!=null) 
  ...
  document.getElementById('txtPR2').focus();	
	...
	document.getElementById('txtCOMISSAO_REPRESENTANTE').focus();
}	
...
ajax.criar('ajax/ajaxGRUPOS_VENDA.php?acao=lerREGS&ativos='+
...
if ( ajax.terminouLER() ) {
  ...
  titulo.innerHTML = '&nbsp;&nbsp;<font size="+1">Grupos de venda</font>&nbsp;&nbsp;&nbsp;&nbsp;';
...
function incluirREG() {
showAJAX(1);
ajax.criar('ajax/ajaxGRUPOS_VENDA.php?acao=incluirREG', desenhaJanelaREG);
}
...
showAJAX(1);
ajax.criar('ajax/ajaxGRUPOS_VENDA.php?acao=editarREG&vlr=' + id, desenhaJanelaREG);
}
...
if (nomeCMP!='todos')         {
  ...
  /* tudo ok, aciona gravacao do registro, concatena campos em uma variavel */
	showAJAX(1);
	ajax.criar('ajax/ajaxGRUPOS_VENDA.php?acao=gravar&vlr=' + cmps, gravou);
}
...
if ( ajax.terminouLER() ) {
  ...
  if (resp.indexOf('OK')!=-1)   {
    document.getElementById('SELECAO').value="";
  	...
  	window.top.document.getElementById('infoTrab').value = 'frmGRUPOS_VENDA:GRAVOU=' + cID
  	lerREGS();
  }
  else
...
ajax.criar('ajax/ajaxGRUPOS_VENDA.php?acao=mudarSITUACAO&vlr=' + id, '', 0);
if (ajax.ler().indexOf('ok')==-1) 
  alert('Erro ao gravar!!! \n\n' + ajax.ler());
  
window.top.document.getElementById('infoTrab').value = 'frmGRUPOS_VENDA:GRAVOU=' + id
lerREGS();  
}
