...
<form id="frmBAIRROS" name="frmBAIRROS" autocomplete="off" action="" >
  ...
  <td width="<?php echo $_SESSION['largIFRAME']; ?>" height="<?php echo $_SESSION['altIFRAME']; ?>">
    ...
    <table cellspacing="0" cellpadding="0" border="1" width="75%" bgcolor="white" style="text-align:left;">
      ...
      <tr><td><table width="100%" cellpadding="0" cellspacing="2"><tr>
        <td width="80%"><span class="lblTitJanela" id="lblTITULO">&nbsp;&nbsp;Bairros</span></td>
...
aCMPS[0]='txtNOME;Digite o nome do bairro';
aCMPS[1]='txtMUNICIPIO;Identifique a cidade do bairro';
...
ajax.criar('ajax/ajaxBAIRROS.php?acao=lerREGS', desenhaTabela);
}
...
if ( ajax.terminouLER() ) {
  ...
  titulo.innerHTML = '&nbsp;&nbsp;<font size="+1">Bairros</font>&nbsp;&nbsp;&nbsp;&nbsp;';
...
function incluirREG() {
showAJAX(1);
ajax.criar('ajax/ajaxBAIRROS.php?acao=incluirREG', desenhaJanelaREG);
}
...
showAJAX(1);
ajax.criar('ajax/ajaxBAIRROS.php?acao=editarREG&vlr=' + id, desenhaJanelaREG);
}
...
if (nomeCMP!='todos')         {
  ...
  /* tudo ok, aciona gravacao do registro, concatena campos em uma variavel */
	showAJAX(1);
	ajax.criar('ajax/ajaxBAIRROS.php?acao=gravar&vlr=' + cmps, gravou);
}
...
if ( ajax.terminouLER() ) {
  ...
  if (resp.indexOf('OK')!=-1)   {
    document.getElementById('SELECAO').value="";
  	...
  	window.top.document.getElementById('infoTrab').value = 'frmBAIRROS:GRAVOU=' + cID
  	lerREGS();
  }
  else
...
ajax.criar('ajax/ajaxBAIRROS.php?acao=excluirREG&vlr=' + id, '', 0);

if (ajax.ler().indexOf('ok')==-1) 
  alert('Erro ao excluir!!! \n\n' + ajax.ler());
  
window.top.document.getElementById('infoTrab').value = 'frmBAIRROS:GRAVOU=' + id
lerREGS();  
}
