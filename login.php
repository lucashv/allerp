...
<body style="HEIGHT: 100%; width:100%;" onload="Muda_CSS();ColocaFocoCmpInicial();lerTITULO();">
    ...
    <form id="frmLOGIN" action="">
        ...
        <?php if (Session_id('perdeuSession') == 'sim') { ?>
            <p align="center">
        ...
        function VerCmp(nomeCMP) {
            ...
            if (typeof nomeCMP != 'string') {
                if (ajax.terminouLER()) {
                    ...
                    if (resp[0].indexOf('INEXISTENTE') != -1) {
                        lblNOME.style.color = 'red';
                    } else
                        lblNOME.style.color = 'blue';
                }
                return;
            }
                ...
                for (i = 0; i < aCmps.length; i++) {
                    ...
                    switch (cmp) {
                        case 'txtNUMERO':
                            if ((vlr == ''))
                                erro = 1;
                            if ((lblNOME.innerHTML == "") || (lblNOME.innerHTML == "INEXISTENTE"))
                                erro = 1;

                            break;

                        case 'txtSENHA':
                            if (vlr.toLowerCase() != cSENHA.toLowerCase())
                                erro = 1;
