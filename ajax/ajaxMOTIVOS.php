...
if ($acao=='mudarSITUACAO') {
  ...
  mysql_query("update motivos_cancelamento set ativo=case ativo when 'S' then 'N' else 'S' end where numero=$id") 
    or  die (mysql_error());
...
if ($acao=='gravar') {
  ...
  if ($id=='') 
    $sql = "insert into motivos_cancelamento(descricao, ativo) values('$cmps[0]', 'S')";
  
  else  
    $sql = "update motivos_cancelamento set descricao='$cmps[0]' where numero=$id"; 
...
IF ($acao=='lerREGS') {
  ...
  $sql  = "select numero, descricao, ativo ".
