...
if ($acao=='mudarSITUACAO') {
  ...
  mysql_query("update seguros_tipos set ativo=case ativo when 'S' then 'N' else 'S' end where numreg=$id") 
    or  die (mysql_error());
...
if ($acao=='gravar') {
  ...
  if ($id=='') 
    $sql = "insert into seguros_tipos(nome, ativo) values(upper('$cmps[0]'), 'S')";
  
  else  
    $sql = "update seguros_tipos set nome=upper('$cmps[0]') where numreg=$id"; 
