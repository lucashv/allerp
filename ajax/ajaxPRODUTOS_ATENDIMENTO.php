...
if ($acao=='mudarSITUACAO') {
  ...
  mysql_query("update produto_atendimento set ativo=case ativo when 'S' then 'N' else 'S' end where numero=$id") 
    or  die (mysql_error());
...
if ($acao=='gravar') {
  ...
  if ($id=='') 
    $sql = "insert into produto_atendimento(nome, ativo) values(upper('$cmps[0]'), 'S')";
  
  else  
    $sql = "update produto_atendimento set nome=upper('$cmps[0]') where numero=$id"; 
