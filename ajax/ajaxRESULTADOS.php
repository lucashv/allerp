...
if ($acao=='mudarSITUACAO') {
  ...
  mysql_query("update resultados_indicacoes set ativo=case ativo when 'S' then 'N' else 'S' end where numreg=$id") 
    or  die (mysql_error());
...
if ($acao=='gravar') {
  ...
  if ($id=='') 
    $sql = "insert into resultados_indicacoes(descricao, ativo) values(upper('$cmps[0]'), 'S')";
  
  else  
    $sql = "update resultados_indicacoes set descricao=upper('$cmps[0]') where numreg=$id"; 
...
IF ($acao=='lerREGS') {
  ...
  $sql  = "select numreg, descricao, ativo ".
          " from resultados_indicacoes " .
  ...
  $largura2 = $_SESSION['largIFRAME'] * 0.8;
	...
	$header = "$largura1 px,Número|$largura2 px,descricao";
...
IF ( $acao=='incluirREG' || $acao=='editarREG'  ) {
  ...
  if ($acao!='incluirREG')   {
    $sql  = "select numreg, descricao, ativo " .
            "from resultados_indicacoes rep ".
            "where numreg=$vlr ";
