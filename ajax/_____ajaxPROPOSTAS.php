...
IF ($acao=='verENTPROP') {
  ...
  $sql  = "select ent.numreg, ent.valor as vlrCONTRATO, ent.idREPRESENTANTE, rep.nome as nomeREPRESENTANTE, ent.vlrADESAO, ent.vlrRECEBIDO, ".
          ...
          "where datediff(now(), dataop)<=200 and ent.cpf='$cpf' and ent.idTIPO_CONTRATO=$tipoCONTRATO and ifnull(ent.sequencia,0)=0;";
  ...
  if ( mysql_num_rows($resultado)==0 ) $resp='nenhuma';
    ...
    if (mysql_num_rows($resultado)==1) {
      ...
      $resp="somenteUMA$row->numreg;$row->idREPRESENTANTE;$row->nomeREPRESENTANTE;$row->vlrCONTRATO;$row->vlrADESAO;$row->vlrRECEBIDO";
    }
    else { 
      $resp='';  
...
/*****************************************************************************************/
if ($acao=='devolucao') {                  
  mysql_query("update propostas set jaENVIADA=0, pendente='S' where sequencia=$vlr") or  die (mysql_error());
...
IF ($acao=='lerREGS') {
   ...
   if ($palavra=='') $criterio =  "where dataCADASTRO between '$dataINI' and '$dataFIN' " ; 
   else {
      $cmp = $_REQUEST['cmp'];
...
IF ($acao=='futuras') {
  ...
  $largura4 = $_SESSION['largIFRAME'] * 0.05;
	...
	$header = "$largura1 px,Parcela|$largura4 px,Vencimento|$largura4 px,Valor|$largura2 px,Situação|$largura1 px,Pagamento".
              "|$largura2 px,Período apuração|$largura1 px,Data repasse|$largura4 px,Extrato|$largura3 px,&nbsp;";
  ...
  while ($row = mysql_fetcH_object($resultado)) {
      ...
      if ($row->ordem==1) {
        if ($row->pagaNoAdiantamento==1) {
          ...
          if ($row->marcadaParaPagarAdiantamento==0 ) {
            $linkBAIXAR="$row->ordem<font color=blue>&nbsp;ADIANTAR</font>";
            $funcaoBAIXAR='adiantar(:idFUTURA)';
          }else  {
            $linkBAIXAR="$row->ordem<font color=red>&nbsp;Ñ ADIANTAR</font>";
    ...
    $lin = "<tr title='Arquivo de baixa= $row->nomearq' onmouseover=\"this.style.backgroundColor='#a9b2ca';this.style.cursor='pointer';\" " . 
            "onmouseout=\"this.style.backgroundColor='#E6E8EE';\"   >" . 
            "<td align=\"left\" onclick='$funcaoBAIXAR' $largura1>$linkBAIXAR</td>".
            "<td align=center $largura4>$row->dataVENCIMENTO</td>".
            "<td align=\"right\" $largura4>$vlr&nbsp;</td>".
            "<td align=\"center\" $largura2>$row->situacao</td>".
            "<td align=\"right\" $largura1>$vlrPAGO&nbsp;&nbsp;</td>".                        
            "<td align=\"left\" $largura2>$row->periodoAPURACAO</td>".
            "<td align=\"center\" $largura1>$dataRELATORIO $adto</td>".
            "<td align=\"right\" $largura4>$row->extrato</td>".
            "<td align=\"center\" onclick='$funcao' $largura3>$link</td>".
            "</tr>";
