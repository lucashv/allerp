    ...
    while ($row = mysql_fetcH_object($resultado)) {
      if ($row->tipoPGTO=='VALE CRÉDITO')  
        mysql_query("update creditos_descontos set pagoVALE_CREDITO=0, datapagoVALE_CREDITO=null, oppagoVALE_CREDITO=null " . 
                    "where numVALE_CREDITO=$row->infoCHEQUE") or die( 'reg pgto vale credito _1' . '<br>'.mysql_error() );
    }
  }
    ...
    if ($tipoPGTO=='CHEQUE') 
      ...
      if ($vlrDESCONTADO - $vlrPAGO< 0.1)
        ...
        mysql_query("update creditos_descontos set pagoVALE_CREDITO=1, datapagoVALE_CREDITO='$dataOPERACAO', oppagoVALE_CREDITO=$logado " . 
                    "where numVALE_CREDITO=$pgto[1]") or die( 'reg pgto vale credito _2' . '<br>'.mysql_error() );      
      else
        mysql_query("update creditos_descontos set pagoVALE_CREDITO=0, datapagoVALE_CREDITO=null, oppagoVALE_CREDITO=null " . 
                    "where numVALE_CREDITO=$pgto[1]") or die( 'reg pgto vale credito _3' . '<br>'.mysql_error() );
...
//  $temFormasPgto = trim($cmps[2]=='') ? 0 : 1;
  ...
  for ($f=0; $f<count($pgtos); $f++) {
    ...
    if ($tipoPGTO=='CHEQUE') 
      ...
      // registra que vale credito foi pago
      mysql_query("update creditos_descontos set pagoVALE_CREDITO=1, datapagoVALE_CREDITO='$dataOPERACAO', oppagoVALE_CREDITO=$logado " . 
                  "where numVALE_CREDITO=$pgto[1]") or die( 'reg pgto vale credito _4' . '<br>'.mysql_error() );      
    }
  ...
  if ($bloquearGRAVACAO=='') {
    switch ($acao) { 
      case 'incluirCAIXA':
        ...
        break;      
      case 'editarCAIXA':
        $resp=str_replace('TITULO_JANELA', "Editar operação do caixa nº $vlr  ",$resp);    
        $resp=str_replace('texto_botao', '[ F2=gravar ]',$resp);
        $resp=str_replace('readonly', '',$resp);
        break;
    }
