...
if ($acao=='gravar') {
  ...
  $pagar1a = $_REQUEST['pagar1a']=='true' ? 'S' : 'N';  

  if ($id=='') 
    $sql = "insert into tipos_contrato(descricao,idOPERADORA, ativo, vlradesao, cpf_cnpj,qtde1,qtde2,perc1,qtde3,qtde4,".
              " perc2,qtde5,qtde6,perc3,vlrPRODUCAO, vidas1, vidas2,vidas3,vidas4,vidas5,vidas6,vidas7,vidas8,vidas9,vidas10, pagarPRIMEIRA) ".
            ...
            " $cmps[17], $cmps[18], $cmps[19], $cmps[20], $cmps[21], $cmps[22], '$pagar1a'); ";
  else  
    $sql = "update tipos_contrato set descricao='$descricao', idOPERADORA=$operadora, vlradesao=$adesao, cpf_cnpj=$tipo, ".
          ...
          " vidas5=$cmps[17], vidas6=$cmps[18], vidas7=$cmps[19], vidas8=$cmps[20], vidas9=$cmps[21], vidas10=$cmps[22], pagarPRIMEIRA='$pagar1a' ".
            " where numreg=$id"; 
...
IF ( $acao=='incluirREG' || $acao=='editarREG'  ) {
  ...
  if ($acao!='incluirREG')   {
    $sql  = "select tip.numreg, tip.descricao, op.nome as nomeOPERADORA, idOPERADORA, vlrADESAO, cpf_cnpj, ".
            ...
            " vidas7, vidas8, vidas9, vidas10, pagarPRIMEIRA ".
            "from tipos_contrato tip ".
            "left join operadoras op ".
            '   on op.numreg=tip.idOPERADORA '. 
    ...
    if ($row->pagarPRIMEIRA=='S') $resp=str_replace('checkedPRIMEIRA', 'checked', $resp);
    else $resp=str_replace('checkedPRIMEIRA', '', $resp);
  }    
  else {
