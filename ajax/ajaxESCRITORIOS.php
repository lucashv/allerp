...
if ($acao=='padrao') {
  ...
  mysql_query("update escritorios set escritorioCENTRAL=0")  or  die (mysql_error());
  mysql_query("update escritorios set escritorioCENTRAL=1 where numero=$id")  or  die (mysql_error());
...
if ($acao=='mudarSITUACAO') {
  ...
  mysql_query("update escritorios set ativo=case ativo when 'S' then 'N' else 'S' end where numero=$id") 
    or  die (mysql_error());
...
if ($acao=='gravar') {
  ...
  if ($id=='') 
    $sql = "insert into escritorios(nome, ativo) values('$cmps[0]', 'S')";
  
  else  
    $sql = "update escritorios set nome='$cmps[0]' where numero=$id"; 
...
IF ($acao=='lerREGS') {
  ...
  $sql  = "select numero, nome, ativo, escritorioCENTRAL ".
          " from escritorios " .
  ...
  while ($row = mysql_fetcH_array($resultado, MYSQL_NUM)) {
    ...
    $padrao='';
    if ($row[3]==1) $padrao="<font face=arial style='font-size:14px'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b><br>** ESCRITÓRIO CENTRAL **</b></font>";
