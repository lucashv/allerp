...
<ul id="CM1" class="SimpleContextMenu_MAIOR">
  ...
  <li><a href="javascript:definir();">Definir como escritório central</a></li>
</ul>

<form id="frmESCRITORIOS" name="frmESCRITORIOS" autocomplete="off" action="" >
...
aCMPS[0]='txtNOME;Digite a descrição do escritório';
...
ajax.criar('ajax/ajaxESCRITORIOS.php?acao=lerREGS&ativos='+
...
if ( ajax.terminouLER() ) {
  ...
  titulo.innerHTML = '&nbsp;&nbsp;<font size="+1">Escritórios</font>&nbsp;&nbsp;&nbsp;&nbsp;';
...
function incluirREG() {
showAJAX(1);
ajax.criar('ajax/ajaxESCRITORIOS.php?acao=incluirREG', desenhaJanelaREG);
}
...
showAJAX(1);
ajax.criar('ajax/ajaxESCRITORIOS.php?acao=editarREG&vlr=' + id, desenhaJanelaREG);
}
...
if (nomeCMP!='todos')         {
  ...
  /* tudo ok, aciona gravacao do registro, concatena campos em uma variavel */
	showAJAX(1);
	ajax.criar('ajax/ajaxESCRITORIOS.php?acao=gravar&vlr=' + cmps, gravou);
}
...
if ( ajax.terminouLER() ) {
  ...
  if (resp.indexOf('OK')!=-1)   {
    document.getElementById('SELECAO').value="";
  	...
  	window.top.document.getElementById('infoTrab').value = 'frmESCRITORIOS:GRAVOU=' + cID
  	lerREGS();
  }
  else
...
ajax.criar('ajax/ajaxESCRITORIOS.php?acao=mudarSITUACAO&vlr=' + id, '', 0);
if (ajax.ler().indexOf('ok')==-1) 
  alert('Erro ao gravar!!! \n\n' + ajax.ler());
  
window.top.document.getElementById('infoTrab').value = 'frmESCRITORIOS:GRAVOU=' + id
lerREGS();  
}
...
ajax.criar('ajax/ajaxESCRITORIOS.php?acao=padrao&vlr=' + id, '', 0);
if (ajax.ler().indexOf('ok')==-1) 
  alert('Erro ao gravar!!! \n\n' + ajax.ler());
