...
mysql_query("delete from entregaspropostas") or die(mysql_error());
mysql_query("delete from caixa") or die(mysql_error());
...
$sql  = "select idtipocontrato, date_format(datacadastro, '%Y%-%m%-%d %H:%i') as dataGRAVAR, vlrtotal as vlrcontrato, idrepresentante, cpfcontratante " .
        "from propostas " .
        " where date_format(datacadastro, '%Y%m%d') >= '20111101'  ";

$resultado = mysql_query($sql, $conexao) or die (mysql_error());
while ($row = mysql_fetcH_object($resultado)) {
  $sql = "insert into caixa(entousai, dataop, idoperacao, valor, opresponsavel) ".
          " values('E', '$row->dataGRAVAR', 1, $row->vlrcontrato, 5)";
