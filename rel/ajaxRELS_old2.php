  ...
  if ($formatoREL==1) {
    ...
    if ($tipoREL==1) {  
      $sql  = "select lig.data, lig.numreg, date_format(lig.data, '%d/%m/%y') as dataMOSTRAR, lig.corretorEXCEL, lig.telefones, ".
              ...
              " lig.obs, 'IndicaþÒo' as tabela, sit.descricao as situacao  " .
...
//              "  order by ifnull(lig.tipo, 1),data desc ";
    ...
    $headers=
  	  "                          Tipo do                                                                                                     Origem do   |".
  	  "Data      Situação        contato        Corretor                  Nome                      Telefone(s)        Produto               Contato     |".
      str_repeat('-', 160 );
  ...
  //*****************************************************************************************************************************
      ...
      $sql  = "select lig.data, date_format(lig.data, '%d/%m/%y') as dataMOSTRAR, lig.telefones, lig.email, ".
              ...
              " lig.idINDICACAO, ind.nome as nomeINDICACAO, 'Indicação' as tabela " .
              "from indicacoes_atendimento lig ".
              "left join representantes repre ".
              "    on repre.numero=idCORRETOR ".
              "left join produto_atendimento ope ".
  ...
  if ($tipoREL==1) $relacionou='Plantão';
  else if ($tipoREL==2) $relacionou='Ligações telefônicas (plantão)';
  else if ($tipoREL==3) $relacionou='Indicações';
  else if ($tipoREL==4) $relacionou='Atendimento Presencial';
  else if ($tipoREL==5) $relacionou='Ligações extra plantão';


  if ($operadora!='') $titulos="Produto: $nomeOPERADORA ($operadora)";
  if ($repre=='9999') {
    $titREL = "$relacionou no período: $dataIniMostrar a $dataFinMostrar   ";
    $sql = str_replace('@criterioREPRE', '', $sql);
  }
  else if ($repre=='') {
    $titREL = "$relacionou no período: $dataIniMostrar a $dataFinMostrar  Corretor: $nome";
    $sql = str_replace('@criterioREPRE', " and corretorEXCEL like '%$nome%' " , $sql);
  }
  else {
    $titREL = "$relacionou no período: $dataIniMostrar a $dataFinMostrar  Corretor: $nomeREPRE";
  ...
  if ($formatoREL==2 || $formatoREL==3 || $formatoREL==5) {
    ...
    else if ($formatoREL==5) $tabelaAPOIO_NOME[0]='SEM SITUAÃAO DEFINIDA';
...
//die($sql);
  ...
  if ($formatoREL!=4) { 
    ...
    if ($tipoREL==4) {
    	$info=str_pad(' TOTAL ligaþ§es (ligaþ§es perdidas nÒo sÒo consideradas): ', 60, ' ', 0) .'  ' .
            str_pad($total, 6, ' ', 0) . "\n";
    	$info2=str_pad(' TOTAL presenþas : ', 60, ' ', 0) .'  ' .
            str_pad($total_2, 6, ' ', 0) . "\n";
      fwrite($Arq, "\n"  );
      fwrite($Arq, $info );
      fwrite($Arq, $info2 );
    	...
    	$info=str_pad(' TOTAL (ligaþ§es perdidas nÒo sÒo consideradas): ', 60, ' ', 0) .'  ' .
            str_pad($total, 6, ' ', 0) . "\n";
      fwrite($Arq, "\n"  );
      fwrite($Arq, $info );
    }
...
if ($acao=='cheques') {
  ...
  $titREL = "Cheques $palavra no período: $dataIniMostrar a $dataFinMostrar";
  $titulos="";
  
  $headers=
    '                                                                 Data do   Data da                                                            |'.
	  "Banco                                     N¦ cheque       Valor  cheque    operaþÒo  CPF/CNPJ            Nome                       OperaþÒo|".
    str_repeat('-', 150 );
...
if ($acao=='caixa_agrupadores') {
  ...
  $titulos="período: $dataIniMostrar a $dataFinMostrar";
  
  $headers=
    '                                                                                                                                                                                                   |'. 
    '                                                    Saldo                                 |'. 		
	  'Data   OperaþÒo                                    anterior       SaÝda     Entrada         |'.
    str_repeat('-', 90);
...
if ($acao=='boletos') {
  ...
  $titREL = "Boletos com vencimento período: $dataIniMostrar a $dataFinMostrar";
  $titulos="";
  
  $headers=
	  'N¦ boleto             Data vencimento  Valor R$  N¦ operaþÒo de caixa  Pago?                                         |'.
    str_repeat('-', 130   );
...
if ($acao=='caixa_detalhado') {
  ...
  $headers=
    ...
    '                                                                                    Valor         Percent            Saídas                                     |'. 		
	  'Data   Corretor/Funcionário   Operação                                    Valor  recebido        AllCross   Saídas  Cheques   Cartão  Cheques     Vale  Dinheiro|'.
    str_repeat('-', 160);
...
//die($sql);
  ...
  while ($regCAIXA = mysql_fetcH_object($resultado)) {
    ...
    if ($mostrarPGTOS) {
      ...
      while ($pag = mysql_fetcH_object($pags) )  {
        ...
        if ($tipo=='CHEQUE') {
          ...
          $strPAG = 'BOLETO: '.
  	  							" Nº $pag->infoCHEQUE   Valor: $valorPAGO_2   Vencimento: $pag->dataCHEQUE   $boletoPAGO"; 
      ...
      if ($valeGERADO!=-1) {
        $strPAG = '     CRÉDITO CHEQUE CORRETOR N¦: '.
                  str_pad($valeGERADO, 6, ' ', 0).'  '.
                  ' Corretor: '.$reprevaleGERADO; 
    ...
    $entOUSai=$regCAIXA->entOUsai;
		...
		// idENTREGA=='', significa que nao ha entrega vinculada, o registro Ú uma operacao de cx simples
...
*/
      ...
      // se operacao caixa envolve uma conta cujo envovlido Ú um CORRETOR, considera que o numero Ú de um CORRETOR obviamente
...
//Data   Corretor/Funcionßrio   OperaþÒo                                    Valor  recebido        AllCross   SaÝdas  Cheques   CartÒo  Cheques     Vale  Dinheiro|'.
//99/99  xxxxxxxxxxxxxxx (999)  xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  9999,99   9999,99  9999,99 (999%)  e999,99  t999,99  f999,99  X999,99  3999,99   d999,99
  ...
  if ($valeGERADO!=-1) {
    $strPAG = '     CRÉDITO CHEQUE CORRETOR Nº: '.
              str_pad($valeGERADO, 6, ' ', 0).'  '.
              ' Corretor: '.$reprevaleGERADO; 
...
if ($acao=='creditos') {
  ...
  if ($tipoREL=='2') $filtro='Todos os vale créditos ';
  else if ($tipoREL=='1') $filtro='Créditos/descontos ';
  else if ($tipoREL=='3') $filtro='Vale créditos nÒo pagos';
  else if ($tipoREL=='4') $filtro='Vale créditos pagos';

  if ($tipoBUSCA=='2') {$cmp='data';$titCMP='Data de registro';}
  else  {$cmp='pagarVALE';$titCMP='Data para pagar';} 

  $titREL = "$filtro $titCMP no período: $dataIniMostrar e $dataFinMostrar ";

  // tipoREL=1 , creditos/descontos
  if ($tipoREL=='1') { 
    $headers = 
        "                                     Registrado                                                  Valor                                    |".          
        "Corretor                             em:         Tipo  DescriþÒo                                 R$       Contratante(s)                  |".
       str_repeat('-', 130);
  //     xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  99/99/99    X     xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  9999,99  xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    ...
    $headers = 
        ...
        "Corretor                             em:         pagar     Tipo  DescriþÒo                                 R$       R$       Desconto (%)|".
       str_repeat('-', 140);
  ...
  $sql  = "select cx.numreg as idCAIXA, cx.idOPERACAO, cre.numero, cre.descricao, ifnull(rep.nome, '') as nomeREPRESENTANTE, " .
          ...
          " where date_format($cmp, '%Y%m%d') between '$dataINI' and '$dataFIN' @criterioREPRE @criterioTIPO   and ifnull(cre.excluido,0)=0 AND (alterado2_excluido1 IS NULL OR alterado2_excluido1 = '') " .          
...
//    $pago= $row->pagoVALE_CREDITO==1 ? 'Sim' : 'NÒo';
...
if ($acao=='protocolo') {
  ...
  $headers= 
  "Protocolo de envio de propostas (Pßgina YY)                $nomeEMPRESA ||".         
  "Data do envio ....: $data |".
  "Valor total ......: R$ $vlrTOT   |".
  "Total de contratos: R$ $contratos                          --------------------   ------------------------------------------ |".
  "Total de vidas....: R$ $xxxxvidas                          $xxxxxxnomeOPERADORA   $nomeEMPRESA ||||".
  "N¦  Operadora             Tipo contrato         Nº da proposta  Corretor              Segurado                        Qtde vidas   Valor da fatura  Operação   |".
  "--------------------------------------------------------------------------------------------------------------------------------------------------------------";
...
IF ($acao=='verPROP') {
  ...
  if ( mysql_num_rows($resultado)==0 ) $resp='erro;Proposta nÒo cadastrada';
  else {
    $resp=$row->sequencia;

    if ($row->jaNaLista!='nao') $resp='erro;Proposta jß adicionada Ó lista';
    else if ($row->cancelada=='S') $resp='erro;Proposta cancelada';
    else if ($row->pendente=='S') $resp='erro;Proposta com pendÛncia';
    else if ($row->jaENVIADA<>0) $resp="pergunta;$row->sequencia";
//    else if ($row->jaENVIADA<>0) $resp="erro;Proposta jß enviada";
...
if ($acao=='confirmacoes') {
  ...
  $titREL = "Confirmações $infoADTO no período ($cmp2): $dataIniMostrar e $dataFinMostrar  " .
    ($relCREDITOS ? " - Créd/déb período $dataIniMostrar_VALES e $dataFinMostrar_VALES" : ''); 
  if ($nomeOPERADORA!='') $titulos = "Operadora: $nomeOPERADORA ($idOPERADORA)";

  $headers = 
      "                  Data de   Data do                                                           Base de    Parcela   Comissão|".          
      "Produto           cadastro  extrato   Corretor                   Segurado                     cálculo      (%)       (R$)  |".
     str_repeat('-', 127);
...
//      xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx9999999,99hhhhhhhhhhhh9999999,99
  ...
  $sql = "select prop.qtdeMENS, date_format(fut.DataPgtoParcela, '%d/%m/%y') as DataPgtoParcela, fut.numREG as idFUTURA, prop.idTipoContrato as idPRODUTO, ifnull(prod.descricao, '* erro *') as nomePRODUTO, ".
         ...
         "  concat('Banco: ',ban.nome,' (',repre.idBANCO, ')    Agência: ', repre.agencia, ".
          " '    Operação:', repre.operacao, '    Conta: ', repre.num_conta, '    Favorecido: ', repre.favorecido) as dadosCONTA, ". 
...
//      if (! $temALGO) continue;
    ...
    if ($row->comiNaProposta!='' && $row->comiNaProposta!='0') {
      if ($row->qtdeVIDAS >= $row->vidas1 && $row->qtdeVIDAS <= $row->vidas2) {
        ...
        if ($row->ordem=='2') $perc=$row->p2a_6;  
        if ($row->ordem=='3') $perc=$row->p3a_6;
        if ($row->ordem=='4') $perc=$row->p4a_6; 
        if ($row->ordem=='5') $perc=$row->p5a_6;
        
        if ($row->ordem>5 && $row->qtdeMENS=='3')
        { 
        	$perc=$row->pVITALICIA_6;
      ...
      if ($row->qtdeVIDAS >= $row->vidas3 && $row->qtdeVIDAS <= $row->vidas4) {
        ...
        if ($row->ordem>5 && $row->qtdeMENS=='3') $perc=$row->pVITALICIA_7;
      ...
      if ($row->qtdeVIDAS >= $row->vidas5 && $row->qtdeVIDAS <= $row->vidas6) {
        ...
        if ($row->ordem>5 && $row->qtdeMENS=='3') $perc=$row->pVITALICIA_8;
      ...
      if ($row->qtdeVIDAS >= $row->vidas7 && $row->qtdeVIDAS <= $row->vidas8) {
        ...
        if ($row->ordem>5 && $row->qtdeMENS=='3') $perc=$row->pVITALICIA_9;
      ...
      if ($row->qtdeVIDAS >= $row->vidas9 && $row->qtdeVIDAS <= $row->vidas10) {
        ...
        if ($row->ordem>5 && $row->qtdeMENS=='3') $perc=$row->pVITALICIA_10;
    ...
    // se nao especificado (NO CONTRATO) qual comissao de representante usar, usa a comissao especificada no reg do representante
      ...
      if ($row->qtdeVIDAS >= $row->vidas1 && $row->qtdeVIDAS <= $row->vidas2) {
        ...
        if ($row->ordem>5 && $row->qtdeMENS=='3') $perc=$row->pVITALICIA_1;
      }
      if ($row->qtdeVIDAS >= $row->vidas3 && $row->qtdeVIDAS <= $row->vidas4) {
        ...
        if ($row->ordem>5 && $row->qtdeMENS=='3') $perc=$row->pVITALICIA_2;
      }
      if ($row->qtdeVIDAS >= $row->vidas5 && $row->qtdeVIDAS <= $row->vidas6) {
        ...
        if ($row->ordem>5 && $row->qtdeMENS=='3') $perc=$row->pVITALICIA_3;
      }
      if ($row->qtdeVIDAS >= $row->vidas7 && $row->qtdeVIDAS <= $row->vidas8) {
        ...
        if ($row->ordem>5 && $row->qtdeMENS=='3') $perc=$row->pVITALICIA_4;
      }
      if ($row->qtdeVIDAS >= $row->vidas9 && $row->qtdeVIDAS <= $row->vidas10) {
        ...
        if ($row->ordem>5 && $row->qtdeMENS=='3') $perc=$row->pVITALICIA_5;
      }          
    }
    $listar=true;
    ...
    // uma vez que a 1a mensalidade Ú paga/rateada no ato da entrega da proposta 
    ...
    if ($listar) {
	  ...
	  //die($perc);
      if ($row->marcadaADIANTAMENTO==1) 
      	$perc = (int)$perc - 10; 
		$baseCALCULO = $row->valor;
      //echo $perc;
      
      $comissao = $row->valor * ($perc/100); 
      ...
      */
	  ...
	  $totCOMISSAO += $row->valor;
      $totG_COMISSAO += $row->valor;

      $totCOMISSAO_2 += $row->valor * ($perc/100);
      $totG_COMISSAO_2 += $row->valor * ($perc/100);
...
if ($acao=='vendas') {
  ...
  //************************************************************************************************
  if ($tipoREL=='1') {
    $titREL = "Vendas no período: ($data) $dataIniMostrar e $dataFinMostrar $infoADD ";

    $headers = 
        "                                    Data de                                           Valor do   Valor     Valor      Valor    Data de   Qtde   Num| ".          
        "Operadora        Produto            cadastro  Corretor               Segurado         contrato  recebido  produção   plantão  envio    Vidas  prop|".
       str_repeat('-', 155);
  //    xxxxxxxxxxxxxxx  xxxxxxxxxxxxxxxxx  99/99/99  xxxxxxxxxxxxxxx (9999)  xxxxxxxxxxxxxxxxxxxx  99999,99  99999,99  99999,99  99999,99  99/99/99  9999     
    ...
    $sql = "select ifnull(prop.qtdeVIDAS,0) as qtdeVIDAS, ifnull(opa.nome, '* ERRO *') as nomeOPERADORA, ifnull(contra.descricao, '* ERRO *') as tipoCONTRATO, " .
           ...
           " AND (alterada2_excluida1 IS NULL OR alterada2_excluida1 = '')" .
           //" AND pendente <> 'S' " .
           " AND ifnull(pendente, 'N') <> 'S'  " .
...
//die($sql);
  ...
  //************************************************************************************************
  if ($tipoREL=='2') {
    $titREL = "Propostas canceladas no período: ($data) $dataIniMostrar e $dataFinMostrar $infoADD ";
  ...
  //************************************************************************************************
  //************************************************************************************************
  // tipo de relatorio PENDENTES
  //************************************************************************************************
  //************************************************************************************************
  if ($tipoREL=='6') {
    $titREL = "Propostas pendentes no período: ($data) $dataIniMostrar e $dataFinMostrar $infoADD ";
  ...
  //     xxxxxxxxxxxxxx  xxxxxxxxxxxxxxxxx  99/99/99  xxxxxxxxxxxxxxxxxxxx (9999)  xxxxxxxxxxxxxxxxxxxx            99999,99        
    ...
    $sql = "select ifnull(prop.qtdeVIDAS,0) as qtdeVIDAS, ifnull(opa.nome, '* ERRO *') as nomeOPERADORA, ifnull(contra.descricao, '* ERRO *') as tipoCONTRATO, " .
           ...
           "left join representantes repre " .
           "   on repre.numero = prop.idREPRESENTANTE " .
           " where date_format($cmp, '%Y%m%d') between '$dataINI' and '$dataFIN' @criterioREPRE @criterioOPERADORA @criterioTIPO and prop.pendente='S' ".
           " order by repre.nome, contratante";      // prop.idOPERADORA, prop.idTipoContrato 
  ...
  if ($tipoREL=='3') {
    ...
    $headers = 
        ...
        "Colocação  Corretor                             Contratos    produção    contrato    recebido     plantão|".
       str_repeat('-', 110);
  ...
  if ($tipoREL=='4') {
    ...
    $headers = 
        ...
        "contrato                             Contratos    produção    contrato    recebido     plantão|".
       str_repeat('-', 80);
  ...
  if ($tipoREL=='5') {
    ...
    $headers = 
        ...
        "Corretor                                            Contratos    produção    contrato    recebido|".
       str_repeat('-', 120);
...
if ($acao=='contratos_erros') {
  ...
  $headers = 
      "SequÛncia Contratante                                         Problema                               |".
     str_repeat('-', 120);
//    999999     xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  xxx      
  ...
  $sql = "select opa.numreg as numOPERADORA, contra.numreg as numTIPOCONTRATO, prop.contratante, prop.sequencia, " .
        " prop.operadoraEXCEL, prop.tipoContratoEXCEL ".
         ...
         "left join tipos_contrato contra " .
         "   on contra.numreg = prop.idTipoContrato " .
          " where contra.numreg is null or opa.numreg IS NULL or contra.numreg = '' or  opa.numreg = '' ".
          " order by contratante";
...
fwrite($Arq,  "Relatório emitido em: $hoje as $agora  por: $adicional[0] ($adicional[1])        PÁGINA: $pagina \n");
fwrite($Arq,  str_repeat('=', 80) . "\n");
$lin += 3;
...
$titRELCRED = "Créditos/débitos registrados no período: $dataIniMostrar e $dataFinMostrar ";
...
if ($linCRED+6>55) {
  ...
  $txtCRED .=  "Relatório emitido em: $hoje as $agora, por: $adicional[0] ($adicional[1])       PÁGINA: $pagina \n";
  $txtCRED .=  str_repeat('=', 80) . "\n";
  $linCRED += 3;
}
