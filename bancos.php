...
<form id="frmBANCOS" name="frmBANCOS" autocomplete="off" action="" >
...
aCMPS[0]='txtNUMERO;Digite o nº do banco';
aCMPS[1]='txtNOME;Digite o nome do banco';
...
ajax.criar('ajax/ajaxBANCOS.php?acao=lerREGS', desenhaTabela);
}
...
if ( ajax.terminouLER() ) {
  ...
  titulo.innerHTML = '&nbsp;&nbsp;<font size="+1">Bancos</font>&nbsp;&nbsp;&nbsp;&nbsp;';
...
function incluirREG() {
showAJAX(1);
ajax.criar('ajax/ajaxBANCOS.php?acao=incluirREG', desenhaJanelaREG);
}
...
showAJAX(1);
ajax.criar('ajax/ajaxBANCOS.php?acao=editarREG&vlr=' + id, desenhaJanelaREG);
}
...
if (nomeCMP!='todos')         {
    ...
    var label = cCMP.replace('txt', 'lbl'); 		
      ...
      case 'txtNUMERO':
        /* verifica se ha banco diferente do atual registrado com mesmo numero */
        ...
        if (document.getElementById('tituloEDICAO').innerHTML.toLowerCase().indexOf('incluir')!=-1)
      	...
      	showAJAX(1);
      	ajax.criar('ajax/ajaxBANCOS.php?acao=verDUPLICIDADE&vlr=' + 
        ...
        if (ajax.ler().indexOf('jaCAD')!=-1) {
          alert('Já há um banco cadastrado com este número.');
          return;
        }
        break;
 		}
  ...
  /* tudo ok, aciona gravacao do registro, concatena campos em uma variavel */
	showAJAX(1);
	ajax.criar('ajax/ajaxBANCOS.php?acao=gravar&vlr=' + cmps, gravou);
}
...
if ( ajax.terminouLER() ) {
  ...
  if (resp.indexOf('OK')!=-1)   {
    document.getElementById('SELECAO').value="";
  	...
  	window.top.document.getElementById('infoTrab').value = 'frmBANCOS:GRAVOU=' + cID
  	lerREGS();
  }
  else
...
ajax.criar('ajax/ajaxBANCOS.php?acao=mudarSITUACAO&vlr=' + id, '', 0);
if (ajax.ler().indexOf('ok')==-1) 
  alert('Erro ao gravar!!! \n\n' + ajax.ler());
  
window.top.document.getElementById('infoTrab').value = 'frmBANCOS:GRAVOU=' + id
lerREGS();  
}
