...
<div id="divCOMISSOES" class="cssDIV_ESCONDE"></div>
...
lJanVALORES= document.getElementById('divCOMISSOES').getAttribute(propCLASSE)!='cssDIV_ESCONDE';
...
document.getElementById("divCOMISSOES").innerHTML='';
document.getElementById("divCOMISSOES").setAttribute(propCLASSE, "cssDIV_ESCONDE");
ColocaFocoCmpInicial();
}
...
ajax.criar('ajax/ajaxCOMISSAO_ADESAO.php?acao=lerREGS&ativos='+
...
if ( ajax.terminouLER() ) {
  ...
  centerDiv( 'divEDICAO' );
  centerDiv( 'divCOMISSOES' );
  VerificaAcaoInicial();
  
  var titulo=document.getElementById('lblTITULO');
  titulo.innerHTML = '&nbsp;&nbsp;<font size="+1">Tipos de comissão sobre adesão</font>&nbsp;&nbsp;&nbsp;&nbsp;';
...
function incluirREG() {
showAJAX(1);
ajax.criar('ajax/ajaxCOMISSAO_ADESAO.php?acao=incluirREG', desenhaJanelaREG);
}
...
showAJAX(1);
ajax.criar('ajax/ajaxCOMISSAO_ADESAO.php?acao=editarREG&vlr=' + id, desenhaJanelaREG);
}
...
if (nomeCMP!='todos')         {
  ...
  /* tudo ok, aciona gravacao do registro, concatena campos em uma variavel */
	showAJAX(1);
	ajax.criar('ajax/ajaxCOMISSAO_ADESAO.php?acao=gravar&vlr=' + cmps, gravou);
}
...
ajax.criar('ajax/ajaxCOMISSAO_ADESAO.php?acao=mudarSITUACAO&vlr=' + id, '', 0);
...
ajax.criar('ajax/ajaxCOMISSAO_ADESAO.php?acao=valores&vlr=' + id+'&desc='+desc, '', 0);

var divCOMISSOES= document.getElementById('divCOMISSOES');  
divCOMISSOES.innerHTML = ajax.ler();
divCOMISSOES.setAttribute(propCLASSE, 'cssDIV_VALORES');
...
showAJAX(1);
ajax.criar('ajax/ajaxCOMISSAO_ADESAO.php?acao=editarCOMISSAO&prod=' + idPRODUTO+'&parcela='+parcela+
...
ajax.criar('ajax/ajaxCOMISSAO_ADESAO.php?acao=padrao&vlr=' + id, '', 0);
if (ajax.ler().indexOf('ok')==-1) 
  alert('Erro ao gravar!!! \n\n' + ajax.ler());
