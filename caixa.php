...
<input type="hidden" id="idUsuarioLogado" name="idUsuarioLogado" value="<?php echo $infoUSUARIO[1] ?>" />
...
var idUsuarioLogado = document.getElementById('idUsuarioLogado').value;
var valorDataCaixa= (document.getElementById('txtDATA').value).split("/");
var valorDataCaixaValidacao = new Date(valorDataCaixa[2], valorDataCaixa[1]-1, valorDataCaixa[0]);
var valorDataCaixaEdicao= (document.getElementById('txtDATA_SEG').value).split("/");
var valorDataCaixaEdicaoValidacao = new Date(valorDataCaixaEdicao[2], valorDataCaixaEdicao[1]-1, valorDataCaixaEdicao[0]);
var dataHojeValidacao = new Date();

//alterando datas para comparacoes
valorDataCaixaValidacao.setFullYear(valorDataCaixaValidacao.getFullYear()+100);
valorDataCaixaEdicaoValidacao.setFullYear(valorDataCaixaEdicaoValidacao.getFullYear()+100);
dataHojeValidacao.setHours(00);
dataHojeValidacao.setMinutes(00);
dataHojeValidacao.setSeconds(00);
dataHojeValidacao.setMilliseconds(00);

/**
console.log("edicao: "+valorDataCaixaEdicaoValidacao);
console.log("edicao time: "+valorDataCaixaEdicaoValidacao.getTime());

console.log("informada: "+valorDataCaixaValidacao);
console.log("informada time: "+valorDataCaixaValidacao.getTime());
console.log("hoje: "+dataHojeValidacao);
console.log("hoje time: "+dataHojeValidacao.getTime());
**/

var dd = dataHojeValidacao.getDate(); 
var mm = dataHojeValidacao.getMonth()+1;//January is 0! 
var yy = dataHojeValidacao.getFullYear()-2000; 
if(dd<10){dd='0'+dd} 
if(mm<10){mm='0'+mm} 

var idUsuarioLiberado = 132;
if(valorDataCaixaEdicaoValidacao.getTime() != valorDataCaixaValidacao.getTime()){
	//Entao é uma ediçao de registros antigos
	if(valorDataCaixaEdicaoValidacao < dataHojeValidacao){
		if(valorDataCaixaValidacao < valorDataCaixaEdicaoValidacao){
			//Unico usuario que pode alterar é a Luciane ID=132
			if(idUsuarioLogado == idUsuarioLiberado){
				alert("ATENÇÃO:\nA data será alterada para uma data anterior a data de cadastro.")
			}else{
				alert("ERRO:\nVocê não tem permissão para alterar para datas\nanteriores a data de cadastro.\nA data será automaticamente alterada para: "+document.getElementById('txtDATA_SEG').value);
				document.getElementById('txtDATA').value = document.getElementById('txtDATA_SEG').value;
			}
		}else if(valorDataCaixaValidacao > valorDataCaixaEdicaoValidacao && valorDataCaixaValidacao < dataHojeValidacao){
			if(idUsuarioLogado == idUsuarioLiberado){
				alert("ATENÇÃO:\nA data será alterada para uma data anterior a data de atual.")
			}else{
				alert("ERRO:\nVocê não tem permissão para alterar para datas\nanteriores a data atual\nFavor utilizar a data de cadastro ou a data atual.\nA data será automaticamente alterada para: "+document.getElementById('txtDATA_SEG').value);
				document.getElementById('txtDATA').value = document.getElementById('txtDATA_SEG').value;
			}
		}
	}else {
		if(valorDataCaixaValidacao < dataHojeValidacao){
			//Unico usuario que pode alterar é a Luciane ID=132
			if(idUsuarioLogado == idUsuarioLiberado){
				alert("ATENÇÃO:\n A data será registrada com uma data anterior a data atual.")
			}else{
				alert("ERRO:\nVocê não tem permissão para incluir registros\ncom datas anteriores a data atual.");
