...
//	if ( strpos($lin, 'Regra:8')!==false ) $PF=true;
...
//    print_r($info);die();
    ...
    if ($parcela==-1) {
      ...
      fwrite($ArqERROS, "\nlinha $contLIN=    $lin ");
      fwrite($ArqERROS, "   -> parcela não é numérica ou faltando \n");
      continue;
    }  
    ...
    if ($ok) {
       ...
       if ( ! is_numeric($numPROPOSTA) ) 
          fwrite($ArqERROS, "   -> Número proposta não é numérico ou faltando \n");
       else if ( ! is_numeric($vlrPAGO) ) 
          fwrite($ArqERROS, "   -> Valor pago não é numérico ou faltando \n");
       else if ( count($dataBAIXA)==2 ) 
          fwrite($ArqERROS, "   -> Data da baixa com problema \n");
    }       
