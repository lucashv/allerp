...
<form id="frmTIPOS" name="frmTIPOS" autocomplete="off" action="" >
...
ajax.criar('ajax/ajaxTIPOS_SEGURO.php?acao=lerREGS&ativos='+
...
if ( ajax.terminouLER() ) {
  ...
  titulo.innerHTML = '&nbsp;&nbsp;<font size="+1">Tipos de seguro</font>&nbsp;&nbsp;&nbsp;&nbsp;';
...
function incluirREG() {
showAJAX(1);
ajax.criar('ajax/ajaxTIPOS_SEGURO.php?acao=incluirREG', desenhaJanelaREG);
}
...
showAJAX(1);
ajax.criar('ajax/ajaxTIPOS_SEGURO.php?acao=editarREG&vlr=' + id, desenhaJanelaREG);
}
...
if (nomeCMP!='todos')         {
  ...
  /* tudo ok, aciona gravacao do registro, concatena campos em uma variavel */
	showAJAX(1);
	ajax.criar('ajax/ajaxTIPOS_SEGURO.php?acao=gravar&vlr=' + cmps, gravou);
}
...
if ( ajax.terminouLER() ) {
  ...
  if (resp.indexOf('OK')!=-1)   {
    document.getElementById('SELECAO').value="";
  	...
  	window.top.document.getElementById('infoTrab').value = 'frmTIPOS:GRAVOU=' + cID
  	lerREGS();
  }
  else
...
ajax.criar('ajax/ajaxTIPOS_SEGURO.php?acao=mudarSITUACAO&vlr=' + id, '', 0);
if (ajax.ler().indexOf('ok')==-1) 
  alert('Erro ao gravar!!! \n\n' + ajax.ler());
  
window.top.document.getElementById('infoTrab').value = 'frmTIPOS:GRAVOU=' + id
lerREGS();  
}
